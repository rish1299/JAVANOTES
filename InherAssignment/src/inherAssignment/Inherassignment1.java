...
class PermanentEmployee extends Employee{
	...
	public void calculateMonthlySalary() {
		...
		double bp = this.basicPay;
		double hrpay = this.hra;
		
		if(experience<3) {
			bp=Math.round(bp+hrpay+(bp*0.00));
			this.setSalary(bp);
			
		}
		else if(experience>=3 && experience<5) {
			bp=Math.round(bp+hrpay+(bp*0.05));
			this.setSalary(bp);
		} 
        else if(experience>=5 && experience<10) {
        	bp=Math.round(bp+hrpay+(bp*0.07));
        	this.setSalary(bp);
		}
        else if(experience>=10) {
        	bp= Math.round(bp+hrpay+(bp*0.12));
        	this.setSalary(bp);
		}		
